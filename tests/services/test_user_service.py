...
def test_update_user_full_name(session: Session):
    user = create_user(session, UserCreate(email="upd@example.com", password="Test1234"))
...
def test_update_user_password(session: Session):
    ...
    user = create_user(session, UserCreate(email="pwd@example.com", password="Test1234"))
